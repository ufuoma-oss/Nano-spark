...
1. 你有一个文件 **HEARTBEAT.md**（默认在工作目录 `~/.nanospark/` 下），里面写的是**每次心跳要问 NanoSpark 的内容**（一段或几段话都行，NanoSpark 会当成一条用户消息）。
2. 系统按你配置的**间隔**（例如每 30 分钟）执行一次：读取 HEARTBEAT.md → 用这段内容去问 NanoSpark → NanoSpark 回复。
3. **发不发到频道** 由配置里的 **target** 决定：
   - **main**：只跑 NanoSpark，不把回复发到任何频道（适合只做「自检」、结果自己看日志或别处）。
   - **last**：把 NanoSpark 的回复发到你**上次和 NanoSpark 对话的那个频道/会话**（例如上次你在钉钉和它聊，这次心跳的回复就发到钉钉）。
...
文件路径默认是 `~/.nanospark/HEARTBEAT.md`。内容就是「每次要问 NanoSpark 什么」，纯文本或 Markdown 都行，NanoSpark 会整体当作一条用户消息。
...
初始化时如果执行过 `nanospark init`（没加 `--defaults`），会提示你是否编辑 HEARTBEAT.md；选是会用系统默认编辑器打开。你也可以之后随时用任何编辑器改这个文件，保存即可，下次心跳会用到新内容。

---

## 第二步：在 config.json 里配置心跳

心跳的**间隔、发到哪、活跃时段**都在 `config.json` 里，路径一般是 `~/.nanospark/config.json`。
...
> 需要「每天 9 点发早安」「每 2 小时问待办并发到钉钉」这类多条任务？用 [CLI](./cli) 的 `nanospark cron create` 做定时任务，不用心跳。
