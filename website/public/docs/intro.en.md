...
This page describes what NanoSpark is, what it can do, and how to get started by
following the docs.
...
NanoSpark is built by the [AgentScope team](https://github.com/agentscope-ai) on
...
## How do you use NanoSpark?

You use NanoSpark in two main ways:

1. **Chat in your messaging apps**
   Send messages in DingTalk, Feishu, QQ, Discord, or iMessage (Mac only); NanoSpark replies
   in the same app and can look things up, manage todos, answer questions —
   whatever the enabled Skills support. One NanoSpark instance can be connected to
   several apps; it replies in the channel where you last talked.

2. **Run on a schedule**
   Without sending a message each time, NanoSpark can run at set times:
   - Send a fixed message to a channel (e.g. “Good morning” to DingTalk at 9am);
   - Ask NanoSpark a question and send the answer to a channel (e.g. every 2 hours
     ask “What are my todos?” and post the reply to DingTalk);
   - Run a “check-in” or digest: ask NanoSpark a block of questions you wrote and
     send the answer to the channel you last used.

After you install, connect at least one channel, and start the server, you can
chat with NanoSpark in DingTalk, Feishu, QQ, etc. and use scheduled messages and check-ins;
...
- **Channel** — Where you talk to NanoSpark (DingTalk, Feishu, QQ, Discord, iMessage, etc.).
  Configure each in [Channels](./channels).
- **Heartbeat** — On a fixed interval, ask NanoSpark a block of text you wrote and
