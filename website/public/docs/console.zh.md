...
**控制台** 是 NanoSpark 内置的 Web 管理界面。运行 `nanospark app` 后，在浏览器中打开
...
- 查看和管理所有聊天会话
- 管理定时任务
- 编辑 NanoSpark 的人设和行为文件
- 开关技能以扩展 NanoSpark 的能力
- 管理MCP客户端
- 修改运行配置
- 配置 LLM 提供商并选择使用的模型
- 管理工具所需的环境变量
...
在这里创建和管理 NanoSpark 按时间自动执行的定时任务。
...
2. 按区域填写表单：
   ...
   - **任务类型及内容** —— 选择 **文本**（发送固定消息）或 **Agent**（向 NanoSpark 提问并
     转发回复），然后填入具体内容。
...
在这里编辑定义 NanoSpark 人设和行为的文件——SOUL.md、AGENTS.md、
...
在这里管理扩展 NanoSpark 能力的技能（如读取 PDF、创建 Word 文档、获取新闻等）。
...
在这里配置 LLM 提供商并选择 NanoSpark 使用的模型。NanoSpark 同时支持云提供商（需要
...
本地提供商显示紫色的 **本地** 标签。
使用前需先安装后端依赖（`pip install 'nanospark[llamacpp]'` 或
...
- 模型来自 Ollama 守护进程（不由 NanoSpark 直接下载）
- 模型列表与 Ollama 自动同步
- 支持热门模型：`mistral:7b`、`qwen3:8b` 等

> 也可以通过 CLI 管理 Ollama 模型：`nanospark models ollama-pull`、`nanospark models ollama-list`、`nanospark models ollama-remove`。详见 [CLI](./cli#ollama-模型)。
...
> **注意：** 云提供商 API Key 的有效性需要用户自行保证，NanoSpark 不会验证。
>
...
在这里管理 NanoSpark 的工具和技能在运行时需要的环境变量（如 `TAVILY_API_KEY`）。
...
> **注意：** 环境变量值的有效性需要用户自行保证，NanoSpark 只负责存储和加载。
>
