...
- **`~/.nanospark`** (the `.nanospark` folder under your home directory)

When you run `nanospark init`, this directory is created automatically. Here's what
you'll find inside:
...
| `jobs.json`          | Cron job list (managed via `nanospark cron` or API)                    |
...
> generic "You are a helpful assistant" prompt. Run `nanospark init` to auto-copy
...
If you don't want to use `~/.nanospark`, you can override the working directory or
specific file names:

| Variable                           | Default         | Meaning                                                                             |
| ---------------------------------- | --------------- | ----------------------------------------------------------------------------------- |
| `NANOSPARK_WORKING_DIR`                | `~/.nanospark`      | Working directory; config, heartbeat, jobs, chats, skills, and memory all live here |
| `NANOSPARK_CONFIG_FILE`                | `config.json`   | Config file name (relative to working dir)                                          |
| `NANOSPARK_HEARTBEAT_FILE`             | `HEARTBEAT.md`  | Heartbeat prompt file name (relative to working dir)                                |
| `NANOSPARK_JOBS_FILE`                  | `jobs.json`     | Cron jobs file name (relative to working dir)                                       |
| `NANOSPARK_CHATS_FILE`                 | `chats.json`    | Chats file name (relative to working dir)                                           |
| `NANOSPARK_LOG_LEVEL`                  | `info`          | Log level for the app (`debug`, `info`, `warning`, `error`, `critical`)             |
| `NANOSPARK_MEMORY_COMPACT_THRESHOLD`   | `100000`        | Character threshold to trigger memory compaction                                    |
| `NANOSPARK_MEMORY_COMPACT_KEEP_RECENT` | `3`             | Number of recent messages kept after compaction                                     |
| `NANOSPARK_MEMORY_COMPACT_RATIO`       | `0.7`           | Threshold ratio for triggering compaction (relative to context window)              |
| `NANOSPARK_CONSOLE_STATIC_DIR`         | _(auto-detect)_ | Path to the console front-end static files                                          |

Example — use a different working dir for this shell:

```bash
export NANOSPARK_WORKING_DIR=/home/me/my_nanospark
nanospark app
```

Config, HEARTBEAT, jobs, memory, etc. will be read/written under
`/home/me/my_nanospark`.
...
| `media_dir`          | string | `~/.nanospark/media` | Directory for received media files  |
...
| `agents.installed_md_files_language` | string \| null | `null`    | Tracks which language's MD files are installed; managed by `nanospark init` |
...
| Field  | Type           | Default | Description                   |
| ------ | -------------- | ------- | ----------------------------- |
| `host` | string \| null | `null`  | Last host used by `nanospark app` |
| `port` | int \| null    | `null`  | Last port used by `nanospark app` |

This is auto-saved every time you run `nanospark app`. Other CLI subcommands
(like `nanospark cron`) use this to know where to send requests.
...
NanoSpark needs an LLM provider to work. You can set it up in three ways:

- **`nanospark init`** — interactive wizard, the easiest way
...
> **Tip:** Run `nanospark init` and follow the prompts — it will list available
...
> NanoSpark does not verify whether the key is correct or has sufficient quota —
...
- **`nanospark init`** — prompts "Configure environment variables?" during setup
...
> keys) are valid. NanoSpark only stores and injects them — it does not verify
> correctness.
...
| Directory                     | Purpose                                                             |
| ----------------------------- | ------------------------------------------------------------------- |
| Built-in (in source code)     | Shipped with NanoSpark — docx, pdf, pptx, xlsx, news, email, cron, etc. |
| `~/.nanospark/customized_skills/` | User-created skills                                                 |
| `~/.nanospark/active_skills/`     | Currently active skills (synced from built-in + customized)         |
...
- `nanospark init` (choose all / none / custom during setup)
- `nanospark skills config` (interactive toggle)
...
NanoSpark has persistent cross-conversation memory: it automatically compresses context and saves key information to Markdown files for long-term retention. See [Memory](./memory.en.md) for full details.

Memory files are stored in two locations:

| File / Directory                | Purpose                                                               |
| ------------------------------- | --------------------------------------------------------------------- |
| `~/.nanospark/MEMORY.md`            | Long-lived key information (decisions, preferences, persistent facts) |
| `~/.nanospark/memory/YYYY-MM-DD.md` | Daily logs (notes, runtime context, auto-generated summaries)         |
...
- Everything lives under **`~/.nanospark`** by default; override with
  `NANOSPARK_WORKING_DIR` (and related env vars) if needed.
...
- LLM providers are configured via `nanospark init` or the console UI.
