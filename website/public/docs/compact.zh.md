...
| 环境变量                           | 默认值   | 说明                                         |
| ---------------------------------- | -------- | -------------------------------------------- |
| `NANOSPARK_MEMORY_COMPACT_THRESHOLD`   | `100000` | 触发自动压缩的 token 阈值（背包容量警戒线）  |
| `NANOSPARK_MEMORY_COMPACT_KEEP_RECENT` | `3`      | 压缩后保留的最近消息数（保持新鲜的对话数量） |
| `NANOSPARK_MEMORY_COMPACT_RATIO`       | `0.7`    | 触发压缩的阈值比例（相对于上下文窗口大小）   |

## 何时触发压缩？

NanoSpark 提供两种压缩方式：**自动挡**和**手动挡** 🚗

### 1. 🤖 自动压缩（接近上下文阈值时）

NanoSpark 就像一个贴心的管家，每次对话前都会检查"背包"还剩多少空间。当可压缩消息的 token 数超过阈值时，自动帮你整理！
