...
async def download_workspace():
    ...
    filename = f"nanospark_workspace_{timestamp}.zip"
...
@router.post(
    ...
    description=(
        ...
        "are left unchanged (e.g. nanospark.db, runtime dirs).  Download packs "
...
async def upload_workspace(  # pylint: disable=too-many-branches
    ...
    tmp_dir = None
    try:
        tmp_dir = Path(tempfile.mkdtemp(prefix="nanospark_upload_"))
        with zipfile.ZipFile(io.BytesIO(data)) as zf:
