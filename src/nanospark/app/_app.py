...
# Console static dir: env, or nanospark package data (console), or cwd.
_CONSOLE_STATIC_ENV = "NANOSPARK_CONSOLE_STATIC_DIR"


def _resolve_console_static_dir() -> str:
    if os.environ.get(_CONSOLE_STATIC_ENV):
        return os.environ[_CONSOLE_STATIC_ENV]
    # Shipped dist lives in nanospark package as static data (not a Python pkg).
...
@app.get("/api/version")
def get_version():
    """Return the current NanoSpark version."""
...
if os.path.isdir(_CONSOLE_STATIC_DIR):
    ...
    @app.get("/nanospark-symbol.svg")
    def _console_icon():
        f = _console_path / "nanospark-symbol.svg"
        if f.is_file():
