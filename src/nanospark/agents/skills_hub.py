...
def _hub_http_timeout() -> float:
    raw = os.environ.get("NANOSPARK_SKILLS_HUB_HTTP_TIMEOUT", "15")
    try:
...
def _hub_http_retries() -> int:
    raw = os.environ.get("NANOSPARK_SKILLS_HUB_HTTP_RETRIES", "3")
    try:
...
def _hub_http_backoff_base() -> float:
    raw = os.environ.get("NANOSPARK_SKILLS_HUB_HTTP_BACKOFF_BASE", "0.8")
    try:
...
def _hub_http_backoff_cap() -> float:
    raw = os.environ.get("NANOSPARK_SKILLS_HUB_HTTP_BACKOFF_CAP", "6")
    try:
...
def _hub_base_url() -> str:
    return os.environ.get("NANOSPARK_SKILLS_HUB_BASE_URL", "https://clawhub.ai")


def _hub_search_path() -> str:
    return os.environ.get(
        "NANOSPARK_SKILLS_HUB_SEARCH_PATH",
        "/api/v1/search",
    )
...
def _hub_version_path() -> str:
    return os.environ.get(
        "NANOSPARK_SKILLS_HUB_VERSION_PATH",
...
def _hub_detail_path() -> str:
    return os.environ.get(
        "NANOSPARK_SKILLS_HUB_DETAIL_PATH",
        "/api/v1/skills/{slug}",
    )
...
def _hub_file_path() -> str:
    return os.environ.get(
        "NANOSPARK_SKILLS_HUB_FILE_PATH",
...
def _http_get(
    ...
    req = Request(
        ...
        headers={
            "Accept": accept,
            "User-Agent": "nanospark-skills-hub/1.0",
        },
    )
