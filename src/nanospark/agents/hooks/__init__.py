...
This package provides hook implementations for NanoSparkAgent that follow
...
Example:
    >>> from nanospark.agents.hooks import BootstrapHook, MemoryCompactionHook
    >>> from pathlib import Path
    >>>
    >>> # Create hooks (they are callables following AgentScope's interface)
    >>> bootstrap = BootstrapHook(Path("~/.nanospark"), language="zh")
