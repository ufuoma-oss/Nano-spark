...
def setup_logger(level: int | str = logging.INFO):
    """Configure logging to only output from this package (nanospark), not deps."""
    ...
    # Only attach handler to our namespace so only nanospark.* logs are printed.
