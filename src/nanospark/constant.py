...
WORKING_DIR = (
    Path(os.environ.get("NANOSPARK_WORKING_DIR", "~/.nanospark"))
    .expanduser()
    .resolve()
)

JOBS_FILE = os.environ.get("NANOSPARK_JOBS_FILE", "jobs.json")

CHATS_FILE = os.environ.get("NANOSPARK_CHATS_FILE", "chats.json")

CONFIG_FILE = os.environ.get("NANOSPARK_CONFIG_FILE", "config.json")

HEARTBEAT_FILE = os.environ.get("NANOSPARK_HEARTBEAT_FILE", "HEARTBEAT.md")
...
LOG_LEVEL_ENV = "NANOSPARK_LOG_LEVEL"

# When True, expose /docs, /redoc, /openapi.json
# (dev only; keep False in prod).
DOCS_ENABLED = os.environ.get("NANOSPARK_OPENAPI_DOCS", "false").lower() in (
    "true",
    "1",
    "yes",
)
...
# Custom channel modules (installed via `nanospark channels install`); manager
...
MEMORY_COMPACT_KEEP_RECENT = int(
    os.environ.get("NANOSPARK_MEMORY_COMPACT_KEEP_RECENT", "3"),
)

MEMORY_COMPACT_RATIO = float(
    os.environ.get("NANOSPARK_MEMORY_COMPACT_RATIO", "0.7"),
)
