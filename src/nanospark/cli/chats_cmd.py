...
def chats_group() -> None:
    ...
    \b
    Common examples:
      nanospark chats list                    # List all chats
      nanospark chats list --user-id alice    # Filter by user
      nanospark chats get <chat_id>           # View details
      nanospark chats create --session-id s1 --user-id u1
      nanospark chats delete <chat_id>        # Delete a chat
    """
...
def list_chats(
    ...
    Examples:
      ...
      nanospark chats list --user-id alice
      nanospark chats list --channel discord
      nanospark chats list --user-id alice --channel discord
    """
...
def get_chat(
    ...
    \b
    CHAT_ID  Chat UUID, obtainable via `nanospark chats list`.
...
def create_chat(
    ...
    Inline creation examples:
      nanospark chats create --session-id "discord:alice" \\
...
def update_chat(
    ...
    \b
    Examples:
      nanospark chats update <chat_id> --name "Renamed Chat"
    """
    base_url = _base_url(ctx, base_url)
...
def delete_chat(
    ...
    \b
    Examples:
      nanospark chats delete 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = _base_url(ctx, base_url)
    with client(base_url) as c:
