# -*- coding: utf-8 -*-
"""nanospark uninstall — remove the NanoSpark environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove NanoSpark PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# NanoSpark" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# NanoSpark\nexport PATH=\"\$HOME/\.nanospark/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove NanoSpark environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL NanoSpark data in {wd}")
    else:
        click.echo(
            "This will remove the NanoSpark Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("NanoSpark uninstalled. Please restart your terminal.")
