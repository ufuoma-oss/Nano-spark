...
def configure_provider_api_key_interactive(
    ...
    if defn.is_local:
        ...
        click.echo(
            "Use 'nanospark models download' to add models, "
            "then 'nanospark models set-llm' to activate.",
        )
        return provider_id
...
def configure_llm_slot_interactive(*, use_defaults: bool = False) -> None:
    ...
    if not eligible:
        if use_defaults:
            click.echo(
                "No LLM provider configured. Run 'nanospark models config' "
                "to configure later.",
            )
            return
        click.echo(
    ...
    if not model and use_defaults:
        click.echo(
            ...
            "Run 'nanospark models config' to set one.",
        )
        return
    ...
    for defn in list_providers():
        ...
        if defn.is_local:
            ...
            if all_models:
                ...
                click.echo("  Use 'nanospark models download' to add models.")
        else:
            if defn.is_custom:
...
def add_provider_cmd(
    ...
    click.echo(
        "  Run 'nanospark models add-model' to add models, "
        "then 'nanospark models config-key' to set the API key.",
    )
...
def download_cmd(
    ...
    \b
    Examples:
      nanospark models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF
      nanospark models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF \\
    ...
    try:
        ...
        click.echo(
            click.style(
                ...
                "Install with: pip install 'nanospark[local]'",
                fg="red",
            ),
        )
        raise SystemExit(1) from exc
    ...
    click.echo(
        ...
        f"  nanospark models set-llm  (select '{backend}' provider)",
    )
...
def list_local_cmd(backend: str | None) -> None:
    ...
    if not models:
        ...
        click.echo("Use 'nanospark models download <repo_id>' to download one.")
        return
...
def ollama_list_cmd() -> None:
    ...
    if not models:
        ...
        click.echo("Use 'nanospark models ollama-pull <model>' to download one.")
        return
...
def ollama_remove_cmd(model_name: str, yes: bool) -> None:
    ...
    Examples:
      ...
      nanospark models ollama-remove qwen2.5:3b -y
    """
    try:
        from ..providers.ollama_manager import OllamaModelManager
    except ImportError as exc:
