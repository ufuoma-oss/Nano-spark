...
If you believe you've found a security issue in NanoSpark, please report it privately.

## Reporting

Report vulnerabilities of the NanoSpark repository:

If you discover a security issue in NanoSpark, please report it to us through the [Alibaba Security Response Center (ASRC)](https://security.alibaba.com/).
...
7. **Environment** (Python version, OS, how NanoSpark is run)
...
- Tested version details (NanoSpark version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to NanoSpark's documented trust boundaries (see below).
- For exposed-secret reports: proof the credential is NanoSpark-owned or grants access to NanoSpark-operated infrastructure/services.
...
- Reports that assume per-user multi-tenant authorization on a shared NanoSpark instance/config.
...
## Alignment with `nanospark init`

The security model and recommended baseline in this document are **aligned with** what users see and accept during **`nanospark init`**. The init flow shows a security notice that covers: single-operator boundary; shared delegated authority when multiple people message the same instance; restricting channels and users (allowlists); using separate config/credentials and OS users or hosts per trust boundary; least privilege and sandboxing; keeping secrets out of the working directory and skill-accessible paths; and reviewing config and skills regularly. When updating either this document or the `SECURITY_WARNING` in `src/nanospark/cli/init_cmd.py`, keep the **concepts and recommendations** consistent so operators get the same message in both places.

## Security & Trust

Security handling is owned by the NanoSpark maintainers. For sensitive reports, use the private advisory or a private channel as above.

## Bug Bounties

NanoSpark is a community open-source project. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly. The best way to help the project right now is by sending PRs or clear, reproducible reports.

## Operator Trust Model

NanoSpark does **not** model one instance as a multi-tenant, adversarial user boundary.

- Authenticated callers to the same NanoSpark instance (same config, same channel workspace) are treated as **trusted operators** for that instance.
...
- **Recommended mode**: one user per machine/host (or per OS user), one NanoSpark config for that user, and one or more agents/skills inside that instance.
- If multiple users need NanoSpark, use one host/OS user (or VPS) per user, or strict isolation; sharing one instance by mutually untrusted users is not the recommended default.
- Skills run with the same privileges as the NanoSpark process; only install and enable skills you trust.


## Trusted Skills Concept

Skills/extensions are part of NanoSpark's **trusted computing base** for an instance.
...
- Public internet exposure of NanoSpark when the docs recommend against it.
- Using NanoSpark in ways that the docs recommend not to.
- Deployments where mutually untrusted/adversarial operators share one NanoSpark instance and config (e.g. reports expecting per-operator isolation for sessions, history, or similar).
...
- Exposed secrets that are third-party or user-controlled credentials (not NanoSpark-owned and not granting access to NanoSpark-operated infrastructure) without demonstrated NanoSpark impact.
- Scanner-only claims against stale or nonexistent paths, or without a working repro.

## Deployment Assumptions

NanoSpark security guidance assumes:

- The host where NanoSpark runs is within a trusted OS/admin boundary.
- Anyone who can modify the NanoSpark working directory and config (including `config.json`) is effectively a trusted operator.
...
NanoSpark's security model is **“personal assistant”** (one trusted operator, potentially many agents/skills), not “shared multi-tenant bus.”

- If multiple people can message the same tool-enabled NanoSpark instance (e.g. a shared channel workspace), they can all steer that agent within its granted permissions.
...
The NanoSpark working directory is treated as **trusted local operator state**.
...
- Keep secrets out of the working directory and skill-accessible paths; this is part of the baseline recommended in `nanospark init`.

## Skills Trust Boundary

Skills are loaded and run **in-process** (or under the same trust boundary) as the NanoSpark runtime and are treated as trusted code.

- Skills can execute with the same OS privileges as the NanoSpark process.
...
For more operational and hardening guidance, see the [documentation](https://nanospark.agentscope.io/) and any security-related docs linked from the repo.

## Runtime Requirements

- **Python**: NanoSpark requires a supported Python version (see [README](README.md)). Use a version with current security updates.
